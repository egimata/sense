...
function initRestabook() {
    ...
    $(window).on("resize", function() {
        if (!resize2) {
            ...
            !window.requestAnimationFrame
                ? setTimeout(setLayerDimensions2, 300)
                : window.requestAnimationFrame(setLayerDimensions2);
        }
    });
    ...
    //  cursor------------------
    $(
        ".hero-wrap , .column-section , .parallax-section , .dark-bg , .cs-content-container , .column-wrap-bg"
    ).on({
        mouseenter: function() {
    ...
    $(
        "a , .btn ,   textarea,   input  , .show-reserv_button , .show-share-btn , .show-cart  , .close-reservation-modal "
    ).on({
        mouseenter: function() {
    ...
    $(
        ".single-slider .swiper-slide , .product-slider .swiper-slide , .events-carousel .swiper-slide , .testimonilas-carousel .swiper-slide , .grid-carousel .swiper-slide , .fs-slider .swiper-slide "
    ).on({
        mouseenter: function() {
    ...
    $(window).on("resize", function() {
        if (!resize3) {
            resize3 = true;
            !window.requestAnimationFrame
                ? setTimeout(setLayerDimensions3, 300)
                : window.requestAnimationFrame(setLayerDimensions3);
        }
    });
    ...
    function videoint() {
        ...
        $(".background-youtube-wrapper").YTPlayer({
            ...
            mute: f,
            callback: function() {
                var a = $(".background-youtube-wrapper").data("ytPlayer")
                    .player;
            }
        });
